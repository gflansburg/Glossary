    ...
    public class GlossaryModuleBase : PortalModuleBase
    {
        ...
        protected string GlossaryPage
        {
            get
            {
                ...
                {
                    string url = Settings[GlossaryController.GLOSSARY_PAGE_KEY].ToString();
                    if (!String.IsNullOrEmpty(url))
                    {
                        int tabId = int.Parse(url);
                        var tabInfo = TabController.Instance.GetTab(tabId, PortalId);
                        url = tabInfo.FullUrl; //.TabPath.Replace("//", "/");
                    }
                    else
                    {
                        url = PortalSettings.HomeDirectory;
                    }
                    return url;
                }
                return String.Empty;
